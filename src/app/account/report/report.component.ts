...
export class ReportComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, 
    ...
    private expenseService: AccountService,
  ...
  getReport(formdata:any): void {
    if (this.reportForm.valid) {
      if (this.reportForm.value.report === 'opt2' && (new Date(this.reportForm.value.startdt) > new Date(this.reportForm.value.enddt))){
        this.toastr.error('Fecha de Inicio no puede ser mayor que fecha final.');
      } else {
    ...
    .subscribe(data => {
      if (data.success === false) {
        ...
        if (formval.report === 'opt1') {
          this.reportTitle = 'para ' + this.datePipe.transform(new Date(), 'MMM y');
        } else if (formval.report === 'opt2') {
            this.reportTitle = 'entre ' + this.datePipe.transform(new Date(formval.startdt), 'd MMM y') + ' y ' + this.datePipe.transform(new Date(formval.enddt), 'd MMM y');
        } else {
          this.reportTitle = 'de todos los gastos '
        }
      }
    });
  }
  ...
  if(confirm('Quiere eliminar este gasto?')){
